...
//
// Why this exists:
// mysql2's execute() (prepared statements / binary protocol) sends each
// bound parameter with an explicit wire type. When a value bound to a
// `LIMIT ?` / `OFFSET ?` placeholder is a string (e.g. "3") instead of a
// real JS number, or is not a clean non-negative integer, TiDB's parser
// rejects the statement with "Incorrect arguments to LIMIT".
//
// Some mysql2/TiDB version combinations reject placeholders in the
// LIMIT/OFFSET position entirely, regardless of type. The only fix that
// works reliably across versions is to validate the value as a finite,
// non-negative integer in JS and inline it directly into the SQL text
// (never parameterize it), while keeping every other value (search
// strings, ids, hashes) as a real `?` placeholder.
//
// This is safe against SQL injection because the value is coerced with
// Number() and checked with Number.isInteger()/range checks before it is
// ever concatenated into the query string. No string, object, or SQL
// fragment can survive that check.
// -----------------------------------------------------------------------
const toSafePositiveInt = (value, fallback, { min = 0, max } = {}) => {
...
export const getQuestionsService = async (filters = {}) => {
  // Get page and limit from filters.
  // If frontend does not send them, use default values.
  // These are validated as finite, non-negative integers so they can be
  // safely inlined into the SQL string for LIMIT/OFFSET (see
  // toSafePositiveInt above for why placeholders are not used here).
  const page = toSafePositiveInt(filters.page, 1, { min: 1 });
  const limit = toSafePositiveInt(filters.limit, 10, { min: 1, max: 100 });
  ...
  // Main list query.
  // LIMIT/OFFSET are validated integers injected directly into the SQL
  // text (NOT bound as ? params) — mysql2's execute() rejects string or
  // improperly-typed values in this position, which is what was causing
  // "Incorrect arguments to LIMIT". Every other value stays parameterized.
  const listSql = `
    SELECT
      q.question_id AS id,
      q.question_hash AS questionHash,
...
export const getSingleQuestionService = async ({ questionHash }) => {
  // Limit answers returned for one question.
  // Validated integer, safe to inline directly (see toSafePositiveInt).
  const answerLimit = toSafePositiveInt(100, 100, { min: 1, max: 500 });
  ...
  // Fetch answers for this question.
  // LIMIT is injected directly (validated integer) instead of bound as a
  // placeholder, for the same reason as getQuestionsService above.
  const answersSql = `
    SELECT
      a.answer_id AS id,
      a.content,
